...
function AgendaWidget() {
  ...
  const copyLink = () => { navigator.clipboard.writeText(availLink).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);}); };
  const addEvent = () => {
    ...
    <div style={{background:G[800],border:`1px solid ${G[600]}44`,borderLeft:`3px solid #5bc4a0`,borderRadius:12,padding:"14px 16px",marginBottom:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:6}}>
          <button onClick={()=>setShowLink(v=>!v)} style={{background:`${G[600]}33`,border:`1px solid ${G[600]}`,borderRadius:7,padding:"4px 10px",color:G[300],fontSize:10,cursor:"pointer"}}>🔗 LINK</button>
          <button onClick={()=>setShowForm(v=>!v)} style={{background:`${G[400]}22`,border:`1px solid ${G[500]}`,borderRadius:7,padding:"4px 10px",color:G[300],fontSize:10,cursor:"pointer"}}>+ EVENTO</button>
          <a href="https://calendar.google.com" target="_blank" rel="noopener noreferrer" style={{fontSize:9,color:G[600],textDecoration:"none",padding:"5px 8px"}}>ABRIR ↗</a>
        </div>
      </div>
      {showLink&&(
        <div style={{background:"#5bc4a011",border:"1px solid #5bc4a033",borderRadius:9,padding:"12px 14px",marginBottom:12}}>
          ...
          <div style={{fontSize:11,color:G[600],marginBottom:8}}>Compartilhe para que alguém veja sua agenda e marque um horário</div>
      ...
      {upcoming.map(ev=>(
        <div key={ev.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:7}}>
          ...
          <div style={{flex:1,background:`${G[700]}55`,borderRadius:7,padding:"6px 10px"}}>
            ...
            {ev.allDay ? <div style={{fontSize:10,color:G[600],marginTop:1}}>Dia inteiro</div> : ev.start&&<div style={{fontSize:10,color:G[600],marginTop:1}}>{ev.start}{ev.end?" — "+ev.end:""}</div>}
          </div>
    ...
    <div style={{position:"fixed",inset:0,background:"#000000cc",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
      <div style={{background:G[800],border:`1px solid ${G[500]}`,borderRadius:16,padding:"28px 32px",width:"100%",maxWidth:560,maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {[[fmt(kpis.fat),"Faturamento",G[300]],[fmt(kpis.fat-kpis.custo),"Resultado",kpis.fat-kpis.custo>=0?"#5bc4a0":"#e07a5f"],[`${pct}%`,"Meta",parseFloat(pct)>=100?"#5bc4a0":parseFloat(pct)>=80?G[400]:"#e07a5f"],[fmt(kpis.caixa),"Caixa",kpis.caixa>=0?G[200]:"#e07a5f"]].map(([v,l,c])=>(
            ...
            {okrs.slice(0,2).map(o=>{
              ...
              return (
                <div key={o.id} style={{marginBottom:6}}>
                  ...
                  <div style={{height:4,background:G[700],borderRadius:2}}><div style={{height:"100%",width:`${p}%`,background:p>=80?"#5bc4a0":G[400],borderRadius:2}}/></div>
                </div>
              );
            })}
          </div>
          ...
          <div style={{marginBottom:14}}>
            ...
            {activeContracts.slice(0,3).map(c=>(
              <div key={c.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:G[200],marginBottom:4}}>
                <span>{c.nome}</span><span style={{color:STATUS_COLORS[c.status]}}>{c.status}</span>
              </div>
            ))}
          </div>
        )}
        {decisions.length>0&&(
          <div>
            ...
            <div style={{background:G[700],borderRadius:8,padding:"10px 12px",fontSize:12,color:G[200]}}>{decisions[0].decisao}<div style={{fontSize:10,color:G[700],marginTop:3}}>{decisions[0].date}</div></div>
          </div>
        )}
...
function OrionAgent() {
  ...
  const send = async () => {
    ...
    const ctx="Você é o ORION — Inteligência Executiva MAXXXI. Especialista em gestão empresarial, finanças, estratégia, compliance e inovação. Responda de forma direta, executiva e em português. Use metodologias modernas (OKR, BSC, PDCA, Lean, 5W2H, Matriz BCG) quando relevante. Empresas do ecossistema: Forme Seguro, Original Fotografia, Doctor Wealth, CDL.";
    try {
  ...
  return (
    <>
      <button onClick={()=>setOpen(v=>!v)} style={{position:"fixed",bottom:28,right:28,width:56,height:56,borderRadius:"50%",background:`linear-gradient(135deg,${G[600]},${G[500]})`,border:`2px solid ${G[400]}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:1,boxShadow:`0 4px 24px ${G[400]}44`,zIndex:300,transition:"all 0.2s"}}>
      ...
      {open&&(
        <div style={{position:"fixed",bottom:98,right:28,width:360,background:G[800],border:`1px solid ${G[500]}`,borderRadius:16,display:"flex",flexDirection:"column",zIndex:300,boxShadow:`0 8px 40px #00000099`,overflow:"hidden"}}>
          <div style={{background:`linear-gradient(135deg,${G[700]},${G[800]})`,padding:"14px 18px",borderBottom:`1px solid ${G[700]}`,display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              <div style={{fontSize:13,fontWeight:700,color:G[100],letterSpacing:"1px"}}>{AGENT_NAME}</div>
      ...
      <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
        ...
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
            ...
            <span style={{fontSize:9,color:G[700]}}>· {AGENT_NAME}</span>
          </div>
      ...
      {allKpis.map(({company,kpis})=>{
        ...
        return (
          <div key={company} style={{background:`${G[700]}55`,border:`1px solid ${G[700]}`,borderRadius:8,padding:"10px 14px",marginBottom:6}}>
            ...
            <div style={{height:4,background:G[700],borderRadius:2}}><div style={{height:"100%",width:`${pct}%`,background:color,borderRadius:2,transition:"width 0.5s"}}/></div>
          </div>
        );
      })}
    </div>
    ...
    <div style={S.box}>
      ...
      {editing&&(
        <div style={{background:G[700],borderRadius:10,padding:"14px",marginBottom:14,display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          <div style={{gridColumn:"1/-1",display:"flex",justifyContent:"flex-end"}}><button style={S.btn} onClick={save}>Salvar KPIs</button></div>
        </div>
      )}
      ...
      <div style={{marginBottom:10}}>
        ...
        <div style={{height:6,background:G[700],borderRadius:3}}><div style={{height:"100%",width:`${Math.min(pct,100)}%`,background:pct>=100?"#5bc4a0":G[400],borderRadius:3,transition:"width 0.5s"}}/></div>
      ...
      {okrs.map(okr=>{
        ...
        return (
          <div key={okr.id} style={{background:G[800],border:`1px solid ${G[700]}`,borderRadius:10,padding:"12px 14px",marginBottom:8}}>
            ...
            <div style={{height:4,background:G[700],borderRadius:2,marginBottom:6}}><div style={{height:"100%",width:`${p}%`,background:color,borderRadius:2,transition:"width 0.5s"}}/></div>
            {okr.krs.map((kr,i)=>(
        ...
        <div style={{background:G[700],borderRadius:10,padding:"14px",marginBottom:12,display:"flex",flexDirection:"column",gap:8}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <div style={{flex:1}}>
              ...
              <select style={{...S.sel,width:"100%"}} value={form.prob} onChange={e=>setForm(p=>({...p,prob:+e.target.value}))}>{[1,2,3,4].map(v=><option key={v} value={v}>{v} — {RISK_LABELS[v-1]}</option>)}</select>
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:9,color:G[500],marginBottom:4}}>IMPACTO (1-4)</div>
              <select style={{...S.sel,width:"100%"}} value={form.impact} onChange={e=>setForm(p=>({...p,impact:+e.target.value}))}>{[1,2,3,4].map(v=><option key={v} value={v}>{v} — {RISK_LABELS[v-1]}</option>)}</select>
            </div>
          </div>
          <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
            <button style={S.ghost} onClick={()=>setShowForm(false)}>Cancelar</button>
...
function AIChat({company,area}) {
  const stKey="mx_chat_"+company+"_"+area;
  const [msgs,setMsgs]=useState(()=>LS.get(stKey,[{role:"ai",text:"Olá! Sou o "+AGENT_NAME+", dedicado a "+company+" / "+area+". Como posso ajudar?"}]));
  const [input,setInput]=useState("");
  const [loading,setLoading]=useState(false);
  const endRef=useRef();
  ...
  const send=async()=>{
    ...
    const newMsgs=[...msgs,{role:"user",text:q}];
    setMsgs(newMsgs);setLoading(true);
    const ctx="Você é o "+AGENT_NAME+" — Inteligência Executiva MAXXXI. Empresa: "+company+", Área: "+area+". Responda de forma direta, executiva e em português. Use metodologias modernas quando relevante.";
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:ctx,messages:newMsgs.slice(-10).map(m=>({role:m.role==="ai"?"assistant":"user",content:m.text}))})});
      const data=await res.json();
      setMsgs(p=>[...p,{role:"ai",text:data.content?.[0]?.text||"Erro ao processar."}]);
  ...
  return (
    <div style={S.box}>
      <div style={S.lbl}>{AGENT_NAME} · {company} / {area}</div>
      ...
      <div style={{display:"flex",gap:8}}>
        <input style={{...S.inp,flex:1}} placeholder={"Pergunte ao "+AGENT_NAME+"..."} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()}/>
...
function Workspace({company,area}) {
  ...
  const TABS=[["ai","🤖 "+AGENT_NAME],["kpi","📊 KPIs"],["okr","🎯 OKRs"],["contratos","📋 Contratos"],["riscos","⚠ Riscos"],["decisoes","🧠 Decisões"],["arquivos","📁 Arquivos"]];
  return (
    <div>
      ...
      {tab==="ai"&&<AIChat company={company} area={area}/>}
...
function CompanyCard({name,hov,logoUrl,onSelect,onDelete,onLogoUpload,onBriefing}) {
  ...
  return (
    <div onClick={onSelect} style={S.card(hov)}>
      ...
      <div style={{height:3,background:G[700],borderRadius:2,marginTop:4}}><div style={{height:"100%",width:`${score}%`,background:color,borderRadius:2}}/></div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#000",color:G[200],fontFamily:"'Inter',system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"10px 28px",borderTop:`1px solid ${G[800]}`,display:"flex",justifyContent:"space-between"}}>
        ...
        <div style={{fontSize:9,color:G[700],letterSpacing:"3px"}}>v5.2</div>
      </div>
